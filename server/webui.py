...
<div id="login" class="card"><img class="login-logo" src="/assets/stokledger-logo.png" alt="StokLedger by ACIS"><h2>Login StokLedger Pro Ultima Web</h2><div class="database-login-panel"><div><small>Database</small><b id="loginDatabasePath">Dikelola server / Railway Volume</b></div></div><div class="row"><input id="username" value="admin" autocomplete="username"><input id="password" type="password" placeholder="Password admin" autocomplete="current-password"><button onclick="doLogin()">Login</button></div><p id="loginMsg"></p></div>
...
<div id="subscriptionAdmin" class="page"><div class="card"><div class="transaction-title"><div><p class="eyebrow">ADMIN WEB</p><h2>Trial & Aktivasi Langganan</h2><p class="muted">Kontrol lisensi instalasi web ini. Aktivasi dilakukan manual oleh Administrator.</p></div><button class="secondary" onclick="loadSubscriptionAdmin()">↻ Refresh</button></div><div class="subscription-kpis"><div class="stat">Status<b id="subStatus">-</b></div><div class="stat">Sisa Hari<b id="subDays">-</b></div><div class="stat">Batas User<b id="subUsers">-</b></div><div class="stat">Berlaku Sampai<b id="subExpiry">-</b></div></div><div class="plan-grid"><div class="plan-card"><h3>6 Bulan</h3><b>Rp1.200.000 / 2 user</b><p class="muted">Add-on user Rp100.000/user per 6 bulan.</p></div><div class="plan-card"><h3>1 Tahun</h3><b>Rp2.000.000 / 2 user</b><p class="muted">Add-on user Rp200.000/user per tahun.</p></div></div><div class="form-section"><div class="field-grid three"><label class="field"><span>Paket</span><select id="subPlan"><option value="HALF">6 Bulan · Rp1.200.000 / 2 user</option><option value="YEAR">1 Tahun · Rp2.000.000 / 2 user</option></select></label><label class="field"><span>Add-on User</span><input id="subAddon" type="number" min="0" value="0"></label><label class="field"><span>Tanggal Mulai</span><input id="subStart" type="date"></label><label class="field"><span>Nama Pelanggan / Perusahaan</span><input id="subCustomer"></label><label class="field"><span>Kunci Admin Aktivasi</span><input id="subAdminKey" type="password" autocomplete="off" placeholder="Kunci khusus pemilik aplikasi"></label><label class="field wide-note"><span>Catatan Admin</span><input id="subNotes"></label></div><div class="action-bar"><button onclick="activateSubscription()">Aktifkan Langganan</button><button class="secondary" onclick="resetWebTrial()">Set Trial Baru 7 Hari</button></div><p id="subMsg"></p></div><div class="card"><h3>Rincian Harga</h3><div id="subPriceInfo" class="muted"></div></div></div></div>
...
function startHeartbeat(){if(heartbeatTimer)clearInterval(heartbeatTimer);if(!token)return;const ping=async()=>{try{await api('/api/session/ping',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})})}catch(e){if(/401|Sesi tidak aktif/i.test(e.message)){stopHeartbeat();localStorage.removeItem('slp_token');token='';document.getElementById('app').classList.add('hidden');document.getElementById('login').classList.remove('hidden')}}};ping();heartbeatTimer=setInterval(ping,5000)}
...
async function logout(){stopHeartbeat();try{if(token)await api('/api/logout',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})})}catch(e){}localStorage.removeItem('slp_token');token='';document.getElementById('app').classList.add('hidden');document.getElementById('login').classList.remove('hidden');await refreshLoginDatabase();window.scrollTo(0,0)}
...
['⌂','Master Departemen','Kelola departemen dan cost center',async()=>{page('departments');await loadDepartments()}],
 ...
 system:{title:'Sistem',description:'Pengaturan perusahaan, pengguna, audit, database, dan desain dokumen.',items:[
  ['★','Trial & Langganan','Admin aktivasi paket web dan add-on user',()=>openSubscriptionAdmin()],
  ['☁','Owner Mobile Cloud','Dashboard dan laporan owner melalui cloud Railway',()=>openOwnerCloud()],
  ...
  ['▣','Lokasi Database & Backup','Lihat file database aktif dan folder backup otomatis',()=>openSystemInfo()],
 ]}
};
...
async function loadSubscriptionAdmin(){try{let x=await api('/api/subscription-admin');subStatus.textContent=x.mode==='SUBSCRIPTION'?(x.plan_name||'Aktif'):x.mode;subDays.textContent=x.days_remaining??0;subUsers.textContent=x.max_users??2;let ex=x.mode==='TRIAL'?x.trial_expires_at:x.expires_at;subExpiry.textContent=ex?new Date(ex).toLocaleDateString('id-ID'):'-';subCustomer.value=x.customer_name||'';subAddon.value=x.addon_users||0;subNotes.value=x.notes||'';subStart.value=new Date().toISOString().slice(0,10);updateSubPrice();await refreshSubscriptionChip()}catch(e){msg(subMsg,e.message,false)}}
...
async function activateSubscription(){if(!confirm('Aktifkan paket langganan ini?'))return;try{let x=await api('/api/subscription-admin/activate',{method:'POST',body:JSON.stringify({plan_code:subPlan.value,addon_users:subAddon.value,starts_at:subStart.value?subStart.value+'T00:00:00+07:00':null,customer_name:subCustomer.value,notes:subNotes.value,admin_key:subAdminKey.value})});msg(subMsg,'Langganan aktif sampai '+new Date(x.expires_at).toLocaleDateString('id-ID')+'.');await loadSubscriptionAdmin()}catch(e){msg(subMsg,e.message,false)}}
async function resetWebTrial(){if(!confirm('Set ulang trial menjadi 7 hari mulai sekarang?'))return;try{await api('/api/subscription-admin/reset-trial',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})});msg(subMsg,'Trial 7 hari telah diaktifkan.');await loadSubscriptionAdmin()}catch(e){msg(subMsg,e.message,false)}}
...
async function registerOwnerCloud(){try{await saveOwnerCloud();await api('/api/owner-cloud/register',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})});msg(ocMsg,'Instalasi berhasil didaftarkan.');await loadOwnerCloud()}catch(e){msg(ocMsg,e.message,false)}}
async function pairOwnerCloud(){try{await api('/api/owner-cloud/pairing-code',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})});msg(ocMsg,'Kode pairing baru dibuat.');await loadOwnerCloud()}catch(e){msg(ocMsg,e.message,false)}}
async function syncOwnerCloud(){try{msg(ocMsg,'Mengirim snapshot dashboard...');await api('/api/owner-cloud/sync',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})});msg(ocMsg,'Sinkronisasi berhasil.');await loadOwnerCloud()}catch(e){msg(ocMsg,e.message,false)}}
