...
async function logout(){stopHeartbeat();try{if(token)await api('/api/logout',{method:'POST',body:JSON.stringify({admin_key:subAdminKey.value})})}catch(e){}localStorage.removeItem('slp_token');token='';document.getElementById('app').classList.add('hidden');document.getElementById('authShell').classList.remove('hidden');showAuthPane('login');await refreshLoginDatabase();window.scrollTo(0,0)}
