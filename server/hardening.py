...
def origin_is_allowed(origin: str, referer: str, host: str, forwarded_proto: str='https') -> bool:
    ...
    if not candidate and referer:
        candidate=normalize_origin(referer)
    # Non-browser clients may omit Origin/Referer. Bearer authentication still applies.
...
def generate_totp_secret(bytes_len: int=20) -> str:
    return base64.b32encode(secrets.token_bytes(max(10,int(bytes_len)))).decode().rstrip('=')
...
def verify_totp(secret: str, code: str, timestamp: int|float|None=None, window: int=1) -> bool:
...
def owner_totp_configured() -> bool:
    return bool(_totp_key(os.environ.get('STOKLEDGER_OWNER_TOTP_SECRET','')))


def verify_owner_totp(code: str) -> bool:
    secret=str(os.environ.get('STOKLEDGER_OWNER_TOTP_SECRET','')).strip()
